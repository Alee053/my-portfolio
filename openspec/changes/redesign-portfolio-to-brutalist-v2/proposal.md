...
The current portfolio features complex animations (GSAP), vibrant colors, 3D elements, and physics simulations (Matter.js), which conflict with a desired brutalist, minimalistic aesthetic. This redesign introduces a straight-to-the-point v2 portfolio in `src/pages/v2/` using the exact UI from `v2-ideas/landing-main/`, adding functional browsers for projects and blogs, while reusing existing Decap CMS setup without breaking v1.

## What Changes

- Copy and adapt the brutalist landing UI from `v2-ideas/landing-main/code.html` to `src/pages/v2/index.astro`, including grid backgrounds, technical annotations, mono fonts, and dashed borders.
...
- Primary: `src/pages/v2/` directory (landing, projects-browser, blogs-browser, blog/[slug].astro).
